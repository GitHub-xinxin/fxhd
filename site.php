...
class Ly_fenxiaohuodongModuleSite extends WeModuleSite {
	...
	public function doMobilePayment(){
		...
		if(empty($user['phone'])){
			$url = $this->createMobileUrl('Register',array('artid'=>$_GPC['artid']));
			header("Location:".$url);
			exit;
		}
	...
	public function doMobileMain(){
		...
		$menu_count = count($menu_info);
		//种类列表
		$kind_list = pdo_getall('ly_fxhd_category',array('uniacid'=>$_W['uniacid']));
		//默认开始显示全部种类
		if(empty($_GPC['kind']) || $_GPC['kind'] == -1){
			$sql = 'SELECT a.id as aid,a.end_time,a.start_time,g.banner,a.title,a.orig_price,a.true_price,a.sketch FROM ims_ly_fxhd_activity AS a LEFT JOIN ims_ly_fxhd_goods AS g ON a.goodid = g.id WHERE a.end_time >'.time().' ORDER BY a.id DESC';
			...
			//根据种类选择活动
			$sql = 'SELECT a.id as aid,a.end_time,a.start_time,g.banner,a.title,a.orig_price,a.true_price,a.sketch FROM ims_ly_fxhd_activity AS a LEFT JOIN ims_ly_fxhd_goods AS g ON a.goodid = g.id WHERE g.category = '.$_GPC['kind'].' AND a.end_time >'.time().' ORDER BY a.id DESC';
			$act_list = pdo_fetchall($sql);
	...
	public function doMobileRegister(){
		...
		//根据传的活动值，确定这个活动
		$artid=$_GPC['artid'];
		$sql='SELECT a.title,a.true_price,a.notice,a.id as aid FROM  ims_ly_fxhd_goods as g LEFT JOIN  ims_ly_fxhd_activity as a ON g.id=a.goodid WHERE g.uniacid='.$_W['uniacid'].' AND a.id ='.$artid;
		$goods=pdo_fetch($sql);
		...
		if($_W['isajax']){
			...
			if(!$is_send){
				...
				if($red_count < $act_info['red_count']){
					//是否即时发送
					$money = rand($act_info['red_min'],$act_info['red_max']);
			...
			if($_GPC['op'] == 'rebate'){		
				...
				if(!empty($one_id)){
					...
					if(!empty($two_id)){
						...
						$temp1['time'] = date('Y-m-d H:m',$temp['pay_time']);
						...
						if(!empty($three_id)){
							...
							$temp2['time'] = date('Y-m-d H:m',$temp['pay_time']);
				...
				$page = $_GPC['page'];
				$temp = array();
				$sql = 'SELECT o.id AS id,u.name,u.phone,o.activityid,o.is_take FROM ims_ly_fxhd_orders AS o LEFT JOIN ims_ly_fxhd_users AS u ON o.userid = u.id WHERE o.uniacid = '.$_W['uniacid'].' AND o.activityid ='.$_GPC['actid'].' AND o.type = 1 ORDER BY o.id DESC';
	...
	public function doMobileTest(){
		var_dump(file_get_contents(IA_ROOT."/addons/ly_fenxiaohuodong/cert/apiclient_cert.pem"));
	}
	//发送红包
